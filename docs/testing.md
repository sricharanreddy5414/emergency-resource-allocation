...
The Emergency Resource Allocation Platform was tested using multiple scenarios to verify resource matching, priority-based allocation, resource exhaustion handling, concurrency protection, event-driven notifications, and monitoring.
...
* Verify that concurrent requests do not allocate the same resource twice.
...
* Verify that CloudWatch records Lambda execution and errors.
...
Therefore, the higher-priority request is processed before the lower-priority request.
...
This protects against double allocation during the tested concurrent scenario.
...
CloudWatch logs confirmed that the EventBridge event was successfully sent.

The event response showed:
...
Verify that a successful allocation triggers an email notification.
...
The SNS notification was successfully delivered to the confirmed email subscription.
...
RESOURCE RESERVED
ALLOCATION CREATED
REQUEST UPDATED: ALLOCATED
EVENTBRIDGE EVENT SENT
SUCCESS
```

These logs provide visibility into the allocation workflow.
...
The alarm is connected to the SNS notification topic.
...
Verify that the Lambda function operates using restricted permissions.

### Security Configuration

The Lambda execution role was configured using least-privilege permissions.
...
After removing unnecessary broad DynamoDB and EventBridge managed policies, the Lambda function was tested again.

The function successfully accessed the required resources without an `AccessDeniedException`.
